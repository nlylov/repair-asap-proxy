                ...
                if (messages.messages) {
                    ...
                    context.conversationHistory = messages.messages
                        ...
                        .map(m => ({
                            ...
                            // userId present = sent by a human (owner/team)
                            // userId absent = sent by workflow/bot/automation
...
async function handleWebhook(event) {
    ...
    let currentMessage = messageBody || message || '';
    ...
    // Only silence the bot when a HUMAN (userId present) sent an outbound message.
    // Bot/workflow messages have no userId — they must NOT trigger the cooldown,
    // otherwise the bot goes silent after its own first reply (major bug).
    const history = contactContext.conversationHistory;
    if (history.length > 0) {
        const lastHumanOutbound = [...history]
            .reverse()
            .find(m => m.direction === 'outbound' && m.userId);
        ...
        if (lastHumanOutbound && lastHumanOutbound.dateAdded) {
            ...
            if (timeSinceOwner < OWNER_COOLDOWN_MS) {
                ...
                logger.info(`Human owner active ${minsAgo}min ago, bot stays silent`, { contactId });
                return {
                    skipped: true,
                    message: '',
    ...
    // 3. Message delivery:
    // GHL workflow handles delivery for all channels:
    //   - Yelp: "Send Yelp message" action uses {{custom_webhook.body.aiResponse}}
    //   - Thumbtack: "Send Thumbtack message" action uses {{custom_webhook.body.aiResponse}}
    //   - SMS: standard SMS action uses {{custom_webhook.body.aiResponse}}
