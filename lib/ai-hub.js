...
async function handleWebhook(event) {
    ...
    // 3. Message delivery:
    // For most channels: GHL workflow sends the message using webhook response body
    // For Thumbtack initial leads: send directly via API (workflow has no SMS action)
    if (channel === 'thumbtack' && contactContext.conversationId && aiResult.message) {
        try {
            await sendResponse({
                contactId,
            ...
            logger.info('Thumbtack initial response sent directly via API', { contactId });
        } catch (sendErr) {
            logger.error('Failed to send Thumbtack response directly', { error: sendErr.message, contactId });
        }
    }
